...
The application LLM can only begin generating a response after all input checks, which may [run in parallel](/configure-guardrails/yaml-schema/guardrails-configuration#iorails-engine), are complete. Additionally, response latency is introduced if you run the guardrail checks on the application LLM's response; the larger the response, the longer it takes to check the response.
