...
title: "Working with Variables & Expressions"
description: "Like Python, Colang supports these fundamental data types: `string`, `int`, `float`, `bool`, `list`, `set`, `dict`, in addition to the Colang specific event, action and flow refere"
---

<a id="working-with-variables-and-expressions"></a>
...
Colang supports evaluation of common Python expressions for simple and compound data types (see [Simple Eval](https://github.com/danthedeckie/simpleeval)):
...
By default, variables defined in a flow have a local scope and are not accessible from outside the flow. One way to enable access to them is by declaring them as flow attributes using the notation shown in [Defining Flows](defining-flows#defining-flows) in the flow definition:
...
```
Next, we learn how to use [Flow control](flow-control#flow-control) to create branching or looping interaction patterns.
