...
This topic demonstrates how to add input rails to a guardrails configuration. As discussed in the previous guide, [Demo Use Case](/configure-guardrails/colang/colang-1/tutorials/3-demo-use-case), this topic guides you through building the ABC Bot.
...
Configure the **general instructions** for the bot. You can think of them as the system prompt. For details, see the [Configuration Reference](/configure-guardrails/configuration-reference#instructions). These instructions configure the bot to answer questions about the employee handbook and the company's policies.
...
Allowing the LLM to comply with this type of request is something we don't want. To prevent jailbreak attempts like this, you can add an input rail that can process the user input before it is sent to the LLM. NeMo Guardrails comes with a built-in [self check input](/configure-guardrails/guardrail-catalog/self-check#self-check-input) rail that uses a separate LLM query to detect a jailbreak attempt. To use it, you have to:
...
Input rails also have the ability to alter the message from the user. By changing the value for the `$user_message` variable, the subsequent input rails and dialog rails work with the updated value. This can be useful, for example, to mask sensitive information. For an example of this behavior, checkout the [Sensitive Data Detection rails](/configure-guardrails/guardrail-catalog/pii-detection#presidio-based-sensitive-data-detection).

## Next

The next guide, [Output Rails](/configure-guardrails/colang/colang-1/tutorials/5-output-rails), adds output moderation to the bot.
