...
This guide builds on the [Hello World guide](/configure-guardrails/colang/colang-1/tutorials/1-hello-world) and introduces the core Colang concepts you should understand to get started with NeMo Guardrails.
...
1. A set of general instructions. These can be [configured](/configure-guardrails/configuration-reference#instructions) using the `instructions` key in *config.yml*.

2. A sample conversation, which can also be [configured](/configure-guardrails/configuration-reference#sample-conversation) using the `sample_conversation` key in *config.yml*.
...
As we can see, the LLM correctly predicted the `express greeting` canonical form. It even went further to predict what the bot should do, which is `bot express greeting`, and the utterance that should be used. However, for the `generate_user_intent` task, only the first predicted line is used. If you want the LLM to predict everything in a single call, you can enable the [single LLM call option](/configure-guardrails/configuration-reference#dialog-rails) in *config.yml* by setting the `rails.dialog.single_call` key to **True**.
...
![Sequence diagram showing the three main steps of processing a user greeting: 1) Computing the canonical form of the user message, 2) Determining the next step using flows, and 3) Generating the bot's response message](../../../../../_static/puml/core_colang_concepts_fig_1.png "Sequence diagram showing the three main steps of processing a user greeting: 1) Computing the canonical form of the user message, 2) Determining the next step using flows, and 3) Generating the bot's response message")
...
This guide provides a detailed overview of two core Colang concepts: *messages* and *flows*. It also looked at how the message and flow definitions are used under the hood and how the LLM is prompted. For more details, see the reference documentation for the [Python API](/run-guardrailed-inference/using-python-apis) and the [Colang Language Syntax](/configure-guardrails/colang/colang-1/colang-language-syntax-guide).

## Next

The next guide, [Demo Use Case](/configure-guardrails/colang/colang-1/tutorials/3-demo-use-case), guides you through selecting a demo use case to implement different types of rails, such as for input, output, or dialog.
