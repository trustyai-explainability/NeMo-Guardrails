...
This guide describes how to add output rails to a guardrails configuration. This guide builds on the previous guide, [Input Rails](/configure-guardrails/colang/colang-1/tutorials/4-input-rails), developing further the demo ABC Bot.
...
NeMo Guardrails comes with a built-in [output self-checking rail](/configure-guardrails/guardrail-catalog/self-check#self-check-output). This rail uses a separate LLM call to make sure that the bot's response should be allowed.
...
![Sequence diagram showing how the self-check output rail works in NeMo Guardrails when processing a response that contains blocked content](../../../../../_static/puml/output_rails_fig_1.png "Sequence diagram showing how the self-check output rail works in NeMo Guardrails when processing a response that contains blocked content")
...
refer to the [Configuration Reference](/configure-guardrails/configuration-reference#output-rails).
...
The next guide, [Topical Rails](/configure-guardrails/colang/colang-1/tutorials/6-topical-rails), adds a topical rails to the ABC bot, to make sure it only responds to questions related to the employment situation.
