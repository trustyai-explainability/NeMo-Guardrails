...
description: "Design policy-based evaluation datasets and interpret compliance results for guardrails configurations."
---

As mentioned in [Evaluate Configuration](/evaluation/evaluate-configuration), we propose a guardrails evaluation methodology based on policies.
This page clarifies the key concepts of a policy-based methodology for guardrails configuration.
...
At the same time, the evaluation of any guardrails configuration requires a dataset of **interactions**. These define the **expected behavior** of the application for specific user inputs, either single-turn or longer interactions (both turns and sequences of actions).
...
We have chosen compliance rate because it is relevant for measuring guardrails effectiveness, while also being simple to understand.
From a machine learning metrics perspective, the compliance rate is similar to accuracy. For example, for a moderation policy it counts all correctly unsafe and blocked interactions and all correctly safe and unblocked interactions from the total considered ones.
...
In addition to the compliance rate, additional metrics are computed to account for the efficiency of a guardrails configuration. Such metrics include the number of LLM calls, invoked actions, total tokens in LLM calls, and overall latency.
...
An [evaluation configuration](/evaluation/evaluate-configuration#evaluation-configuration) contains information about policies, interactions, LLM-as-a-judge models, and latencies.
In this section, we will focus on the best practices for defining these key components of any guardrails evaluation.

As an example, we will consider the guardrails configuration of the [ABC bot](https://github.com/NVIDIA-NeMo/Guardrails/tree/develop/examples/bots/abc).
...
The first step in defining an evaluation configuration is to define the policies that are enforced by the guardrails configuration.
...
The evaluation configuration contains some other policies that are not detailed in this guide (e.g., output moderation or responding to general company information queries).
...
The instruction datasets are also essential for a guardrails evaluation.
...
The simplest way to compute the compliance rate, e.g., if the actual response generated by the tested guardrails configuration is complying with the expected output, is to use a powerful LLM as a judge.
